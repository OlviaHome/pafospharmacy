...
Next.js application
  ...
  ├─ domain: time-window, versioned regular/duty rules, status, filtering, and distance
...
- evaluating the versioned official regular and duty profiles in `Europe/Nicosia`;
- deriving open, on-duty, and on-call meaning across independent regular, ordinary-interval, and duty facts;
...
The current CSV resources establish pharmacy-directory facts and date-only rota assignments. They do not establish per-pharmacy ordinary intervals, exact duty timestamps, open/on-call modes, or coordinates. The importer therefore writes `pharmacies` and `duty_assignments` only; it does not manufacture `availability_intervals`. Separately cited pharmacy-hours orders establish a versioned 2026 regular schedule and a Cyprus Pharmaceutical Services notice establishes a fixed May–September 2026 duty-hours profile. Pure domain evaluators apply those rules at runtime; no generated periods are persisted and the duty assignment row remains date-only. Unexpected records are reported rather than silently discarded. Stable registration numbers and source record identifiers make reruns idempotent. Multi-number house-phone fields retain both their exact decoded source value and every distinct valid normalized number; no number is promoted to the singular convenience field without an unambiguous source value. These House Tel. fields remain in the official snapshot and privileged database model for provenance, but the runtime repository deliberately omits them from its public domain object and client serialization. The Call action continues to use only the separately published pharmacy telephone.
...
The widget is a presentation boundary, not an application data source. Its public API exposes the rendered place's ID/location/viewport and load/error events, but not opening status as structured output. Place Search's `isOpenNow` option filters a new Google search, not this application's known official Place-ID set. Therefore the application must not inspect widget text or shadow DOM, persist its content, or use it to drive the custom opening filter. The application-owned **Open Now** filter instead uses the cited official regular schedule plus active official duty-open periods. A future UI Kit experiment, if approved, remains presentation-only and should instantiate details lazily for an exact-match card after explicit user action, preserving the official list and avoiding 81 homepage requests.
...
3. At a given instant, the domain layer derives status across timed, date-only, and versioned official-rule facts:
   - **Open Now** is true if the supported official regular schedule is open, any active interval has `service_mode = open`, or a supported official assignment is inside the notice's mandatory duty-open period;
...
4. The **Open Now** filter uses the union of the two official physical-opening profiles. Regular closed periods, duty gaps, overnight phone coverage, and unsupported dates are excluded unless another trustworthy active timed fact independently proves opening.
...
- The supported official regular profile is `cyprus-regular-pharmacy-hours-2026`: 1 May–30 September Mon/Tue/Thu/Fri 08:00–13:30 and 16:00–19:30, Wed/Sat 08:00–13:30; 1 October–30 April uses 15:00–18:30 for the second weekday period; Sunday and the explicitly versioned 2026 pharmacy-closure holidays are closed. Unsupported years return unknown rather than extrapolating movable Orthodox holidays.
- The 2026 closure calendar is explicit: 1 January, 6 January, Clean Monday (23 February), 25 March, 1 April, Holy Saturday (11 April), Easter Monday (13 April), Easter Tuesday (14 April), 1 May, Holy Spirit Monday (1 June), 15 August, 1 October, 28 October, 25 December, and 26 December. Movable dates are data for this supported year, not a guessed annual recurrence.
- The regular profile means “should be open according to the official schedule”; it cannot detect exceptional individual closures. No seasonal profile is inferred because the product has no official per-pharmacy seasonal registry.
...
English is the master copy. Every public page lives under a supported locale prefix (`/en`, `/el`, `/ru`, or `/ar`), and Proxy redirects unprefixed pages without duplicating content. A valid explicit `paphos_locale` cookie wins over `Accept-Language`; otherwise supported browser preferences are matched and English is the fallback. The cookie contains only the locale code, uses `SameSite=Lax`, applies site-wide, and expires after at most one year. The selector preserves the equivalent pathname and query string.

Typed locale resources live under `lib/i18n/locales`. Presentation components receive translated messages while schedule, pharmacy identity, location trust, and data-access logic remain language-independent. Arabic routes render the document as `lang="ar" dir="rtl"`; logical CSS properties and bidi isolation preserve Greek/mixed-script source fields, clock times, distances, and stable external URLs without translating official pharmacy identity.
